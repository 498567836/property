		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$repair): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('index?pid='.$repair['id']);?>"><?php echo ($repair["name"]); ?></a></td>
                        ...
                        <td><?php echo (date('Y-m-d H:i:s',$repair["update_time"])); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$repair['id'].'&pid='.$pid);?>">编辑</a>
							<a href="<?php echo U('setStatus?ids='.$repair['id'].'&status='.abs(1-$repair['status']));?>" class="ajax-get"><?php echo (show_status_op($channel["status"])); ?></a>
